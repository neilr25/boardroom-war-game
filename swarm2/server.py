"""FastAPI server for swarm2 — Organic Conversation Boardroom.
...
from shared_llm import EventEmitter, HumanSessionStore, get_session, resume_human
from orchestrator import run_deliberation

app = FastAPI(title="Swarm2 — Organic Conversation Boardroom")
                    ...
                    while True:
                        try:
                            ...
                            if not s or s.status == "complete":
                                break
            w = asyncio.create_task(_write())
            session.status = "running"
            await run_deliberation(session.topic, sid, str(session_dir),
                                  emitter=_emitter, human_session=session)
